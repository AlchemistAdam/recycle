...
if needed. The general use-case would be to obtain a recycler from the
`Recyclers` factory, then obtain an element with `get` (pop it from the stack)
and recycle it with `free` when it is no longer used (push it back unto the
stack). The code example below shows this scenario in Java code.
